...
class StateDB:
    ...
    def get(self, 
pair: str) -> Optional[Dict[str, Any]]:
        cur = self._conn.cursor()
...
def prune_old_state_records(db_path: str, expiry_days: int = 30, logger_local: logging.Logger = None):
    """
    Run pruning once per UTC day.
Uses metadata.last_prune ISO date to prevent
    repeated pruning within same day.
    Deletes rows where ts < cutoff.
       ...
       async with session.get(url, params=params, timeout=timeout) as resp:
        ...
        data = None
            ...
            if attempt == retries:
                ...
                return None
            await asyncio.sleep(backoff * (attempt ** 1.2) + random.uniform(0, 
0.2))
        except Exception as e:
...
class DataFetcher:
   ...
   self._cache = {}  # simple per-run cache
    ...
    async def fetch_candles(self, session: aiohttp.ClientSession, symbol: str, resolution: str, limit: int):
        """
        symbol: product symbol string as from product['symbol'] (e.g.
"BTC_USDT")
        resolution: "15" or "5"
...
def calculate_sma(data: pd.Series, period: int) -> pd.Series:
    return data.rolling(window=period, min_periods=max(2, 
period//3)).mean()
...
def rngfilt(x: pd.Series, r: pd.Series) -> pd.Series:
 ...
 for i in range(1, len(x)):
        ...
        if curr_x > prev_f:
            ...
            f = prev_f if (curr_x + curr_r) > prev_f 
else (curr_x + curr_r)
...
def calculate_smooth_rsi(df: pd.DataFrame, rsi_len: 
int, kalman_len: int) -> pd.Series:
    close = df['close'].astype(float)
    delta = close.diff()
...
pd.Series(0.0, index=df.index)
    ...
    temp2 = temp2.replace([np.inf, 
-np.inf], np.nan).clip(lower=1e-6).fillna(1e-6)
...
def get_last_closed_indices(df_15m: pd.DataFrame, df_5m: pd.DataFrame) -> Tuple[int, int, int]:
    """Return safe indices for last closed candles across 15m 
and 5m timeframes."""
...
def determine_closed_indices(df_15m: pd.DataFrame, df_5m: pd.DataFrame) -> Tuple[int, int, int]:
    # kept for backward compatibility if some modules call it;
...
smooth_rsi_prev = float(smooth_rsi.iloc[prev_i])
    ...
    logger.debug(f"NaN in core indicators for {pair_name}, skipping")
        ...
        if total_range <= 0:
            upper_wick = 0.0;
lower_wick = 0.0; strong_bullish_close = False; strong_bearish_close = False
        else:
            ...
            strong_bullish_close = bullish_candle and 
(upper_wick / total_range) < 0.20
        ...
        ppo_cross_below_zero = (ppo_prev >= 0) and (ppo_curr 
< 0)
 ...
 ppo_above_minus030 = ppo_curr > -0.30
        ...
        mmh_reversal_buy = (mmh_prev3 > mmh_prev2 and mmh_prev2 > mmh_prev1 and mmh_curr > mmh_prev1) # 3 falling, then 1 rising
        mmh_reversal_sell = (mmh_prev3 < mmh_prev2 and mmh_prev2 < mmh_prev1 and mmh_curr < mmh_prev1) # 3 rising, then 1 falling
  ...
  # Original Buy/Sell (PPO-Signal Crossover) set removed.
        
        # New MMH BUY Reversal Condition Set
        buy_mmh_reversal_conds = {
            "mmh_reversal_buy": mmh_reversal_buy,
            "close_below_rma50": close_below_rma50, # RMA50 for 15 minutes BELOW close (CHECK: assuming you mean close > rma50)
            "close_below_rma200": close_below_rma200, # RMA200 for 5 minutes BELOW close (CHECK: assuming you mean close > rma200)
            "magical_hist_curr>0": mmh_curr > 0,
            "cloud_green": (cloud_state == "green"),
            "strong_bullish_close": strong_bullish_close,
        }
        
        # Re-check: "Rma50 for 15 minutes below close" means CLOSE > RMA50, which is `close_above_rma50`. 
        # Similarly, "Rma200 for 5 minutes below close" means CLOSE > RMA200, which is `close_above_rma200`.
        # Correcting the condition names based on common practice (Close > MA)
        ...
        sell_mmh_reversal_conds_corrected = {
            ...
            "close_below_rma50": close_below_rma50, # RMA50 for 15 minutes ABOVE close (means CLOSE < RMA50)
            "close_below_rma200": close_below_rma200, # RMA200 for 5 minutes ABOVE close (means CLOSE < RMA200)
            "magical_hist_curr<0": mmh_curr < 0,
            "cloud_red": (cloud_state == "red"),
            "strong_bearish_close": strong_bearish_close,
        }
        
        # Keeping existing SRSI and PPO(0, 0.11) conditions (60, 61, 62, 63, 64, 65, 66)
        buy_srsi_conds = {
...
async def send_telegram_alert_async(session: aiohttp.ClientSession, message: str) -> bool:
    ...
    if not token or token == 'xxxx' or not chat_id or chat_id 
== 'xxxx':
      ...
      limit_5m = sp.get("limit_5m", 300)
       ...
       df_5m = parse_candles_result(res5)
        ...
        if df_5m is None or len(df_5m) < (min_required_5m + 2):
            logger.warning(f"Insufficient 5m data for {pair_name}: {0 if 
df_5m is None else len(df_5m)}/{min_required_5m + 2}")
            return None
...
async def run_once(send_test: bool = True):
  ...
  expiry_days = int(cfg.get("STATE_EXPIRY_DAYS", 0))
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if send_test and cfg["SEND_TEST_MESSAGE"] and cfg["TELEGRAM_BOT_TOKEN"] != 'xxxx' and 
cfg["TELEGRAM_CHAT_ID"] != 'xxxx':
            ist = pytz.timezone('Asia/Kolkata')
            current_dt = datetime.now(ist)
            formatted_time = current_dt.strftime('%d-%m-%Y @ %H:%M IST')
        ...
        elif send_test and (cfg["TELEGRAM_BOT_TOKEN"] 
== 'xxxx' or cfg["TELEGRAM_CHAT_ID"] == 'xxxx'):
