...
def test_report_age_uses_time_in_file_name():
    ...
    assert co.file_age_reference(Path("2026-03-04.jsonl")).hour == 23      # date-only names unchanged

def test_workflows_use_6_hour_cadence_and_stage_reports():
    root = Path(__file__).resolve().parent.parent / ".github" / "workflows"
    run = (root / "run-bot.yml").read_text(encoding="utf-8")
    assert "(NOW / 900) % 24 ))" in run and "% 16" not in run
    assert "git add --sparse -f reports" in run          # plain `git add` silently skips sparse paths
    assert "--reports-max-age-days 7" in (root / "cleanup-outcomes.yml").read_text(encoding="utf-8")
