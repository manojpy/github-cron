...
Standalone script to trigger Ahead-of-Time compilation for all Numba functions.
Includes both serial and parallel indicators to ensure full cache coverage.
"""
import os
import sys
import time
...
# 2. Import ALL Parallel and Serial Functions
...
try:
    ...
    print(f"❌ Critical Error: Could not import functions. Check src/macd_unified.py path. Error: {e}")
    sys.exit(1)
...
# 4. Compilation Registry (Complete list)
# Format: (Name, Lambda call)
functions_to_compile = [
    ...
    ("_kalman_loop", lambda: _kalman_loop(close_data, 10, 0.1, 0.01)),
    ("_vwap_daily_loop", lambda: _vwap_daily_loop(high_data, low_data, close_data, vol_data, ts_data)),
    ("_rng_filter_loop", lambda: _rng_filter_loop(close_data, close_data, np.zeros_like(close_data))),
...
print(f"🚀 Triggering compilation for {len(functions_to_compile)} indicators...")

for name, task in functions_to_compile:
    try:
        f_start = time.time()
        task() # Triggers the JIT compiler to save to AOT cache
...
print(f"\n{'='*70}")
print(f"🏁 Full AOT Compilation Summary")
print(f"{'='*70}")
...
# Final verification of cache folder
...
if cache_path.exists():
    ...
    print("❌ Error: Cache directory was not created. Check permissions.")
