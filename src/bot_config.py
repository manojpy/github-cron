...
class BotConfig(BaseModel):
    ...
    ENABLE_CHOCH_ALERT: bool = Field(default=False, description="Add 15m Change-of-Character (CHoCH) alert: liquidity sweep of a short-term low/high followed by a displacement close beyond the last opposing minor swing pivot, gated by buy_trend_common/sell_trend_common plus a wick or reversal-pattern condition")
    ...
    CHOCH_CONFIRM_WINDOW_CANDLES: int = Field(default=6, ge=1, le=20, description="Max candles allowed between the liquidity-sweep candle and the structure-break candle. A break found further back than this is rejected as stale")
    CHOCH_ALLOW_SAME_CANDLE_SWEEP: bool = Field(default=False, description="If True, the sweep and the structure break may occur on the same 15m candle. If False (default), the break must be strictly after the sweep candle, removing same-candle sweep/break ambiguity")
    ...
    CHOCH_MIN_BREAK_DISTANCE_ATR: float = Field(default=0.05, ge=0.0, le=2.0, description="Minimum close distance (in ATR_SHORT multiples) beyond the last opposing minor swing pivot required to count as a confirmed structure break")
    ...
    ENABLE_TLR_ALERT: bool = Field(default=False, description="Enable Trendline Reversal (3rd-touch) alerts")
    TLR_FRACTAL_PERIODS: int = Field(default=20, ge=2, le=100, description="Bars on each side used to confirm a swing fractal for TLR trendline anchors (mirrors Auto_Trendline.txt's n1). Separate from CHOCH_SWING_LEN by design — a trendline needs more significant swings than CHoCH's fine-grained structure pivots")
    TLR_TOUCH_TOLERANCE_ATR: float = Field(default=0.25, ge=0.0, le=2.0, description="Distance (in ATR_SHORT multiples) a candle's wick may sit from the projected trendline value and still count as a touch rather than a miss")
    TLR_BREAK_DISTANCE_ATR: float = Field(default=0.05, ge=0.0, le=2.0, description="Minimum close distance (in ATR_SHORT multiples) beyond the trendline, on the wrong side, required to count as a confirmed break rather than noise — mirrors CHOCH_MIN_BREAK_DISTANCE_ATR")
    TLR_BREAK_BODY_RATIO: float = Field(default=0.45, ge=0.0, le=1.0, description="Minimum body-to-range ratio required on a break candle so the trendline break is backed by a displacement move rather than a thin/indecisive close — mirrors CHOCH_MIN_DISPLACEMENT_BODY_RATIO")
    TLR_REQUIRED_TOUCH_NUMBER: int = Field(default=3, ge=2, le=10, description="The touch count at which TLR fires — alert fires exactly on the bar where the running touch count first reaches this number, not on every touch after")
    TLR_TOUCH_STATE_TTL_SEC: int = Field(default=604800, ge=3600, description="TTL (seconds) for persisted per-pair/direction trendline touch-count state in Redis. Default 7 days — trendlines develop over a much longer horizon than the 8-candle CHoCH outcome window")
    TLR_CONFLUENCE_REQUIRED: int = Field(default=3, ge=1, le=5, description="Minimum number of the 5 confluence checks (order block, Fibonacci zone, support/resistance, candlestick pattern, RSI momentum) that must pass for a TLR alert to fire")
    TLR_FIB_ZONE_LOW: float = Field(default=0.5, ge=0.0, le=1.0, description="Lower bound of the Fibonacci retracement zone (as a fraction of the leg from the trendline's older anchor to the extreme reached since) that counts as fib confluence")
    TLR_FIB_ZONE_HIGH: float = Field(default=0.786, ge=0.0, le=1.0, description="Upper bound of the Fibonacci retracement zone — default 0.5-0.786 is the conventional 'golden zone'")
    TLR_SR_LOOKBACK_CANDLES: int = Field(default=300, ge=20, le=2000, description="How many candles back to search for prior swing highs/lows when building support/resistance zones for TLR confluence")
