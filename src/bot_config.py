...
class Constants:
    ...
    RSI_SIGNAL_CROSS_MAX_BUY = 70
    RSI_SIGNAL_CROSS_MIN_SELL = 30
...
class BotConfig(BaseModel):
    ...
    ENABLE_BOS_ALERT: bool = Field(default=False, description="Add 15m Break of Structure (BoS) alert: fires when price closes beyond the most recent confirmed swing high (bullish) or swing low (bearish), confirming trend continuation after a pullback. Gated by buy_common_relaxed_wick/sell_common_relaxed_wick plus PPO/RSI extension cap")
    BOS_SWING_LEN: int = Field(default=5, ge=2, le=20, description="Bars on each side used to confirm a swing pivot for BoS structure — the swing highs/lows the break is measured against")
    BOS_LOOKBACK_CANDLES: int = Field(default=50, ge=10, le=200, description="How many closed 15m candles back from the current candle to scan for qualifying BoS swing structure")
    BOS_MIN_BREAK_DISTANCE_ATR: float = Field(default=0.05, ge=0.0, le=2.0, description="Minimum distance (in ATR_SHORT multiples) the close must exceed beyond the swing level to count as a real structural break rather than noise")
    BOS_PERSISTENCE_CANDLES: int = Field(default=1, ge=0, le=9, description="How many additional closed 15m candles after the break candle to keep the BoS gate valid. 0 = exact-candle-only (fresh break required every cycle)")
