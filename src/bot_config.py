...
class BotConfig(BaseModel):
    ...
    ENABLE_CHOCH_ALERT: bool = Field(default=True, description="Enable Change of Character (CHoCH) structural alerts")
    CHOCH_SWING_LOOKBACK: int = Field(default=12, ge=6, le=50, description="How many 15m bars to scan for swing highs/lows")
    CHOCH_SWEEP_LOOKBACK: int = Field(default=5, ge=2, le=20, description="Max bars after the sweep to confirm a CHoCH break")
    CHOCH_SWING_WINDOW: int = Field(default=2, ge=1, le=5, description="Pivot window size (2 = standard 5-bar swing)")
    CHOCH_MIN_SWEEP_PCT: float = Field(default=0.0005, ge=0.0, le=0.01, description="Min % beyond structural level to count as a liquidity sweep")
    CHOCH_MIN_BREAK_PCT: float = Field(default=0.0005, ge=0.0, le=0.01, description="Min % beyond CHoCH level to count as a structural break")
    CHOCH_DISPLACEMENT_BODY_RATIO: float = Field(default=0.50, ge=0.0, le=1.0, description="Min body/range ratio for the break candle to count as displacement")
    CHOCH_ALLOW_SAME_CANDLE_SWEEP_BREAK: bool = Field(default=True, description="Allow sweep and break on the same candle (wick sweeps, body breaks)")
    CHOCH_MIN_STRUCTURE_PCT: float = Field(default=0.001, ge=0.0, le=0.05, description="Min distance between CHoCH level and sweep level as fraction of price")
    CHOCH_MIN_FVG_PCT: float = Field(default=0.0003, ge=0.0, le=0.01, description="Min FVG gap as fraction of price to count as bonus")
