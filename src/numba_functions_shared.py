...
def rolling_min_max_numba(arr, period):
    ...
    # FIX: Allocate period + 1 to prevent overflow when min_t/max_t reaches 'period'
    min_deque = np.zeros(period + 1, dtype=np.int32)
    max_deque = np.zeros(period + 1, dtype=np.int32)
    min_h = min_t = 0
    max_h = max_t = 0
