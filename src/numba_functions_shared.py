...
def rng_filter_loop(x, r):
    ...
    for i in range(1, n):
        if x[i] > rng_filt[i-1]:
            if x[i] - r[i] < rng_filt[i-1]: rng_filt[i] = rng_filt[i-1]
            else: rng_filt[i] = x[i] - r[i]
        else:
            if x[i] + r[i] > rng_filt[i-1]: rng_filt[i] = rng_filt[i-1]
            else: rng_filt[i] = x[i] + r[i]
    return rng_filt
...
def smooth_range(arr, period, multiplier):
    n = len(arr)
    diff = np.zeros(n)
    for i in range(1, n): diff[i] = abs(arr[i] - arr[i-1])
    alpha = 1.0 / period
    sma_diff = np.zeros(n)
    sma_diff[0] = diff[0]
    for i in range(1, n): sma_diff[i] = alpha * diff[i] + (1.0 - alpha) * sma_diff[i-1]
...
def kalman_loop(arr, gain, k):
    ...
    for i in range(1, n):
        ...
        out[i] = out[i-1] + velocity + error * gain
    return out
...
def rolling_std_welford(arr, window):
    ...
    for i in range(window - 1, n):
        s_sum = 0.0
        for j in range(i - window + 1, i + 1): s_sum += arr[j]
        mean = s_sum / window
        sq_diff = 0.0
        for j in range(i - window + 1, i + 1): sq_diff += (arr[j] - mean) ** 2
...
def calculate_ppo_core(fast_ema, slow_ema):
    ...
    ppo = np.zeros(n)
    for i in range(n):
        if abs(slow_ema[i]) > 1e-10: ppo[i] = ((fast_ema[i] - slow_ema[i]) / slow_ema[i]) * 100.0
    return ppo
...
def calculate_rsi_core(arr, period):
    ...
    for i in range(period + 1, n):
        ...
        if avg_loss > 0: rsi[i] = 100.0 - (100.0 / (1.0 + avg_gain / avg_loss))
        else: rsi[i] = 100.0
    return rsi

# ============================================================================
# MMH COMPONENTS (18-20)
# ============================================================================
...
def calc_mmh_worm_loop(source, stdev_50, n):
    ...
    for i in range(1, n):
        sd = stdev_50[i]
        diff = source[i] - curr
        delta = (np.sign(diff) * sd if abs(diff) > sd else diff) if not np.isnan(sd) else diff
        curr += delta
        out[i] = curr
    return out
...
def calc_mmh_value_loop(temp, n):
    ...
    for i in range(1, n):
        ...
        out[i] = max(-0.9999, min(0.9999, val))
    return out
...
def calc_mmh_momentum_loop(value_arr, n):
    ...
    for i in range(1, n):
        v = value_arr[i]
        t2 = (1.0 + v) / (1.0 - v)
        raw = 0.25 * np.log(t2) if t2 > 0 else 0.0
        out[i] = raw + (0.5 * out[i-1])
    return out
