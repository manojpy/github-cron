...
def print_startup_banner_once() -> None:
    ...
    logger.info(
        ...
        f"🧠 Brain: {'ON' if getattr(cfg, 'ENABLE_BRAIN', False) else 'OFF'} | "
        f"Shadow: {'ON' if getattr(cfg, 'BRAIN_SHADOW_MODE', True) else 'OFF'} | "
        f"Auto-apply: {'ON' if getattr(cfg, 'BRAIN_AUTO_APPLY_DYNAMIC_WEIGHTS', False) else 'OFF'} | "
...
async def process_pairs_with_workers(fetcher: DataFetcher, products_map: Dict[str, dict],
    ...
    if batched_payloads:
        from alerts import dispatch_combined_alerts
        sent = await dispatch_combined_alerts(
            batched_payloads,
            telegram_queue,
            bias_context,          # already computed earlier in this function
            state_db,
            alerts_sent_ref,
            alerts_sent_lock,
            max_alerts_per_run,
