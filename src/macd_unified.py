...
class BotConfig(BaseModel):
    ...
    GITHUB_TOKEN: Optional[str] = Field(default=None, description="GitHub PAT for state storage")
    GITHUB_REPO: Optional[str] = Field(default=None, description="GitHub repo (owner/repo)")
    GITHUB_BRANCH: str = Field(default="main", description="Branch for state files")
...
def validate_runtime_config() -> None:
    ...
    if not cfg.GITHUB_TOKEN or cfg.GITHUB_TOKEN.startswith("__SET_IN_"):
        errors.append("Missing or invalid GITHUB_TOKEN")

    if not cfg.GITHUB_REPO or "/" not in cfg.GITHUB_REPO:
        errors.append("GITHUB_REPO must be in format 'owner/repo'")

    if not cfg.GITHUB_BRANCH:
        errors.append("GITHUB_BRANCH cannot be empty")
