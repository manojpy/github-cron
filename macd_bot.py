...
import math # Added for log and sign functions
...
# Magical Momentum Indicator settings (NEW)
...
def send_test_message():
    ...
    formatted_time = current_dt.strftime('%d-%m-%Y @ %H:%M 
IST')
...
def calculate_rma(data, period):
    """Calculate RMA (Smoothed Moving Average) - 
same as ta.rma in Pine Script"""
...
to a Pandas Series, matching the original index
...
def calculate_magical_momentum(df, period=MOMENTUM_PERIOD, responsiveness=MOMENTUM_RESPONSIVENESS, stdev_period=MOMENTUM_STDEV_PERIOD):
    ...
    # Pine: signal = nz(momentum[1]) (0.0 for the first bar)
    ...
    for i in range(len(close)):
        ...
        # --- 7. Value (Recursive) ---
        # The line `value = 0.5 * 2` (1.0) acts as the base multiplier on the first bar (i=0).
        # On subsequent bars, the base multiplier is the previous calculated value (`value[1]`).
        
        if i == 0:
            base_multiplier = 1.0 # From Pine: value = 0.5 * 2
        else:
            base_multiplier = value_list[-1] # value[1] in Pine
        
        # nz(value[1]) is 0.0 for the first bar, and value_list[-1] otherwise.
        ...
        if np.abs(new_value) >= 1.0:
            ...
            # Use math.log/np.log to implement Pine's math.log
...
def check_pair(pair_name, pair_info, last_alerts):
   ...
   """Check PPO and RMA/Cirrus/SRSI/Momentum conditions for a pair (MODIFIED)"""
  ...
  min_required = 200
        ...
        # Increased limit for Momentum indicator (PERIOD=144, STDEV_PERIOD=50, needs lookback)
        ...
        df_5m = get_candles(pair_info['symbol'], 
"5", limit=limit_5m)
 ...
 print(f"Not enough 5m data for {pair_name} ({len(df_5m) if df_5m is not None else 0}/{min_required_5m})")
    ...
    # ppo_5m, ppo_signal_5m = calculate_ppo(df_5m, PPO_FAST, PPO_SLOW, PPO_SIGNAL, PPO_USE_SMA)
        ...
        # Calculate Smoothed RSI on 
15min timeframe
        smooth_rsi = calculate_smooth_rsi(df_15m)
        
        # Calculate Magical Momentum Indicator (NEW)
...
= smooth_rsi.iloc[-1]
        ...
        # Magical Momentum Hist value (NEW)
   ...
   bullish_candle = close_curr > open_curr
    ...
    debug_log(f"  O:{open_curr:.2f} H:{high_curr:.2f} L:{low_curr:.2f} C:{close_curr:.2f}")
        ...
        if wick_check_valid:
            ...
            debug_log(" 
 Candle range is zero, skipping wick checks.")
 ...
 debug_log(f"Price: ${close_curr:,.2f}")
        ...
        # Magical Momentum Hist Debug Log (NEW)
...
> ppo_signal_curr
        ...
        # ppo_5m_cross_up = (ppo_5m_prev <= ppo_signal_5m_prev) and (ppo_5m_curr > 
ppo_signal_5m_curr)
   ...
   srsi_below_50 = smooth_rsi_curr < 50
        ...
        debug_log(f"  PPO 
15m cross up: {ppo_cross_up}")
        debug_log(f"  PPO 15m cross down: {ppo_cross_down}")
        debug_log(f"  SRSI cross up 50 (Modified): {srsi_cross_up_50}") # MODIFIED
        debug_log(f"  SRSI cross down 50 (Modified): {srsi_cross_down_50}") # MODIFIED
  ...
  debug_log(f"  Upw (Cirrus): {upw_curr}") # Now means GREEN
        ...
        debug_log(f"  Magical Hist > 0 (NEW): {magical_hist_bullish}") # NEW
        debug_log(f"  Magical Hist < 0 (NEW): {magical_hist_bearish}") # NEW
...
and 
              ...
              magical_hist_bearish): # NEW CONDITION ADDED
            current_state = "sell"
        ...
        # 3. NEW BUY ALERT (SRSI CROSS UP 50) (Hist > 0 added)
        elif (srsi_cross_up_50 and # MODIFIED
         ...
         upw_curr and (not dnw_curr) and 
              strong_bullish_close and
              magical_hist_bullish): # NEW CONDITION ADDED
            current_state = "buy_srsi50" # MODIFIED state name
            debug_log(f"\n⬆️ BUY (SRSI 50) SIGNAL DETECTED for {pair_name}!")
            if last_alerts.get(pair_name) != "buy_srsi50": # MODIFIED
                message = f"⬆️ {pair_name} - BUY (SRSI 
50)\nSRSI 15m Cross Up 50 ({smooth_rsi_curr:.2f})\nMagical Hist: {magical_hist_curr:.4f}\nPrice: ${price:,.2f}\n{formatted_time}" # Hist added to message
                send_telegram_alert(message)
            else:
                debug_log(f"BUY (SRSI 50) already alerted for {pair_name}, skipping duplicate") # MODIFIED
                
        # 4. NEW SELL ALERT (SRSI CROSS DOWN 50) (Hist < 0 added)
    
    elif (srsi_cross_down_50 and # MODIFIED
    ...
    strong_bearish_close and
              magical_hist_bearish): # NEW CONDITION ADDED
            current_state = "sell_srsi50" # MODIFIED state name
            debug_log(f"\n⬇️ SELL (SRSI 50) SIGNAL DETECTED for {pair_name}!")
            if last_alerts.get(pair_name) != "sell_srsi50": # MODIFIED
                message = f"⬇️ {pair_name} - SELL (SRSI 50)\nSRSI 15m Cross Down 50 ({smooth_rsi_curr:.2f})\nMagical Hist: {magical_hist_curr:.4f}\nPrice: ${price:,.2f}\n{formatted_time}" # Hist added to message
              
  send_telegram_alert(message)
            else:
                debug_log(f"SELL (SRSI 50) already alerted for {pair_name}, skipping duplicate") # MODIFIED
...
def main():
    """Main function 
- runs once per GitHub Actions execution"""
    print("=" * 50)
    ist = pytz.timezone('Asia/Kolkata')
