    ...
    try:
        if os.path.exists(STATE_FILE):
            with open(STATE_FILE, 'r') as f:
                state = json.load(f)
                # ERROR FIX: De-indented this block by one level
...
def check_pair(pair_name, pair_info, last_alerts):
    ...
    try:
        ...
        debug_log(f"  PPO 15m above 0.20: {ppo_curr > 0.20}")
        debug_log(f"  PPO 15m below -0.20: {ppo_curr < -0.20}")
        ...
        # 2. ORIGINAL SELL: PPO crosses down AND PPO > -0.20 AND ...
            ...
            if last_alerts.get(pair_name) != "sell":
                message = f"🔴 {pair_name} - SELL\nPPO - SIGNAL Crossdown (PPO: {ppo_curr:.2f})\nPrice: ${price:,.2f}\n{formatted_time}"
                send_telegram_alert(message)
            else:
        ...
        # 15m PPO < -0.20 AND 5m PPO Cross Down AND 5m PPO > -0.05 AND other conditions
        elif (ppo_curr < -0.20 and ppo_5m_cross_down and ppo_5m_above_minus005 and macd_below_signal and close_below_rma50 and close_below_rma200 and dnw_curr and (not upw_curr) and strong_bearish_close):
        ...
        # 8. SHORT (-0.11): PPO < Signal AND PPO crosses below -0.11 AND ...
            ...
            current_state = "short_minus011"
            debug_log(f"\n🔴 SHORT (-0.11) SIGNAL DETECTED for {pair_name}!")
            if last_alerts.get(pair_name) != "short_minus011":
                message = f"🔴 {pair_name} - SHORT\nPPO crossing below -0.11 ({ppo_curr:.2f})\nPrice: ${price:,.2f}\n{formatted_time}"
                send_telegram_alert(message)
            else:
                debug_log(f"SHORT (-0.11) already alerted for {pair_name}, skipping duplicate")
                
        # If no signal, but the previous state was a signal, clear the state.
        elif last_alerts.get(pair_name) in ["buy", "sell", "buy_trend", "sell_trend", "long_zero", "short_zero", "long_011", "short_minus011"]:
            current_state = None # Clear state, means no active signal
            debug_log(f"Clear signal for {pair_name}. Previous state: {last_alerts.get(pair_name)}")
