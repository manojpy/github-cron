...
# Special pairs: never go below absolute minimums
SPECIAL_PAIRS = {
    "SOLUSD": {
        "limit_15m": 210,
        "min_required": MIN_15M_CANDLES,
        "limit_5m": 250,
        "min_required_5m": MIN_5M_CANDLES
    }
}
...
def load_state():
    ...
    debug_log("No previous state found")
    return {}
...
def get_product_ids():
    try:
        ...
        if data.get('success'):
            ...
            debug_log(f"Received {len(products)} products")
            for product in products:
                if product.get('contract_type') != 'perpetual_futures':
                    continue
                symbol = product['symbol'].replace('_USDT', 'USD').replace('USDT', 'USD').replace('_', '')
                for pair_name in list(PAIRS.keys()):
                    if symbol == pair_name:
                        ...
                        debug_log(f"Matched {pair_name} -> {product['symbol']}")
                        break
            return True
        else:
...
def check_pair(pair_name, pair_info, last_alerts):
    try:
        ...
        if pair_name in SPECIAL_PAIRS:
            ...
            limit_5m = SPECIAL_PAIRS[pair_name]["limit_5m"]
            min_required_5m = SPECIAL_PAIRS[pair_name]["min_required_5m"]
        else:
            limit_15m = 210
            min_required = MIN_15M_CANDLES
            limit_5m = 210
        ...
        if len(ppo) < 2 or len(smooth_rsi) < 2:
            debug_log("Not enough indicator history for crossover checks")
            return None
        ...
        # --- ALERT LOGIC ---
        # 1. ORIGINAL BUY
        if (ppo_cross_up and ppo_below_020 and close_above_rma50 and close_above_rma200 and
            upw_curr and (not dnw_curr) and strong_bullish_close):
            current_state = "buy"
        ...
        # 2. ORIGINAL SELL
        elif (ppo_cross_down and ppo_above_minus020 and close_below_rma50 and close_below_rma200 and
              dnw_curr and (not upw_curr) and strong_bearish_close):
            current_state = "sell"
        ...
        # 3. SRSI BUY
        elif (srsi_cross_up_50 and ppo_above_signal and ppo_below_030 and close_above_rma50 and close_above_rma200 and
              upw_curr and (not dnw_curr) and strong_bullish_close):
        ...
        # 4. SRSI SELL
        elif (srsi_cross_down_50 and ppo_below_signal and ppo_above_minus030 and close_below_rma50 and close_below_rma200 and
              dnw_curr and (not upw_curr) and strong_bearish_close):
        ...
        # 5. LONG (0)
        elif (ppo_cross_above_zero and ppo_above_signal and close_above_rma50 and close_above_rma200 and
              upw_curr and (not dnw_curr) and strong_bullish_close):
        ...
        # 6. LONG (0.11)
        elif (ppo_cross_above_011 and ppo_above_signal and close_above_rma50 and close_above_rma200 and
              upw_curr and (not dnw_curr) and strong_bullish_close):
        ...
        # 7. SHORT (0)
        elif (ppo_cross_below_zero and ppo_below_signal and close_below_rma50 and close_below_rma200 and
              dnw_curr and (not upw_curr) and strong_bearish_close):
        ...
        # 8. SHORT (-0.11)
        elif (ppo_cross_below_minus011 and ppo_below_signal and close_below_rma50 and close_below_rma200 and
              dnw_curr and (not upw_curr) and strong_bearish_close):
...
def main():
    ...
    print(f"✓ Check complete. {alerts_sent} state updates. ({elapsed:.1f}s)")
    print("=" * 50)
