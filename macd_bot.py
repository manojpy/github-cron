...
def calculate_magical_momentum(df, responsiveness=0.9, period=144):
    ...
    # raw momentum
    # avoid division by zero with worm replaced by small epsilon where zero
...
def check_pair(pair_name, pair_info, last_alerts):
    ...
    try:
        ...
        # Calculate Magical Momentum on 15m
        ...
        # 1. ORIGINAL BUY: PPO crosses up AND PPO < 0.20 AND ... AND hist > 0
        if (ppo_cross_up and 
            ppo_below_020 and 
            ...
            (not np.isnan(hist_curr)) and (hist_curr > 0)): 
            current_state = "buy"
        ...
        # 2. ORIGINAL SELL: PPO crosses down AND PPO > -0.20 AND ... AND hist < 0
        ...
        # 3. SRSI BUY ALERT (SRSI CROSS UP 50) - Only SRSI Cross is the trigger, require hist > 0
        ...
        # 4. SRSI SELL ALERT (SRSI CROSS DOWN 50) - Only SRSI Cross is the trigger, require hist < 0
        ...
        # 5. LONG (0): PPO > Signal AND PPO crosses above 0 AND ... AND hist > 0
        ...
        # 6. LONG (0.11): PPO > Signal AND PPO crosses above 0.11 AND ... AND hist > 0
        ...
        # 7. SHORT (0): PPO < Signal AND PPO crosses below 0 AND ... AND hist < 0
        ...
        # 8. SHORT (-0.11): PPO < Signal AND PPO crosses below -0.11 AND ... AND hist < 0
